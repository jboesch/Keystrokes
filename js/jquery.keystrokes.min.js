(function($){var keystrokes='keystrokes',events=['keydown','keyup'],events_len=events.length;$.event.special[keystrokes]={debug:false,global:{captureInputFields:false,inputs:'input, textarea'},_inputsBound:false,add:function(h,data,namespaces){var $elem=$(this),delegate=$.event.special[keystrokes]._delegate;var stack=$.isArray(data)?data:[data];stack=$.event.special[keystrokes]._addPrivateKeys.call(this,stack,namespaces,h);var tmp_stack=$elem.data('stack');stack=(tmp_stack)?tmp_stack.concat(stack):stack;$elem.data('stack',stack);if(namespaces.length){var i=events_len;while(i--){$elem.bind(events[i]+'.'+namespaces[0],delegate);}}
...
$.event.special[keystrokes]._removeKeyListeners.call(this,name);},setup:function(data,namespaces){var $elem=$(this);$elem.data('keys_down',[]);$elem.data('keys_string',[]);$elem.data('joined',false);},teardown:function(namespaces){var $elem=$(this);$elem.removeData('keys_down');$elem.removeData('keys_string');$elem.removeData('joined');$elem.removeData('stack');},_delegate:function(e){$.event.special[keystrokes]['_'+e.type].call($.event.special[keystrokes],e,this);},_addPrivateKeys:function(data_stack,namespaces,handler){var i=data_stack.length;while(i--){data_stack[i].name=(namespaces[0])?keystrokes+'.'+namespaces[0]:keystrokes;data_stack[i]._namespace=namespaces[0]||keystrokes;delete handler.data;data_stack[i]._handler=handler;}
return data_stack;},_removeKeyListeners:function(name){var $elem=$(this),stack=$elem.data('stack'),stack_len=stack.length;while(stack_len--){if(stack[stack_len]._namespace===name){stack.splice(stack_len,1);}};$elem.data('stack',stack);},_keydown:function(event,elem){var $elem=$(elem),self=this,temp_keys_down=$elem.data('keys_down');if(!this.global.captureInputFields&&$(event.target).not(':input').length==0){if(!this._inputsBound){$(this.global.inputs).bind({"focusin.Keystrokes":function(e){self._clearKeysString(elem);},"focusout.Keystrokes":function(e){self._clearKeysString(elem);}});this._inputsBound=true;}
return;}
temp_keys_down.push(event.keyCode);$elem.data('keys_down',temp_keys_down);$elem.data('joined',false);},_keyup:function(event,elem){var k=event.keyCode,$elem=$(elem),stack=$elem.data('stack'),keys_down=$elem.data('keys_down'),keys_string=$elem.data('keys_string'),stack_len=stack.length;if(!this.global.captureInputFields&&$(event.target).not(':input').length==0){return;}
